...
const languageItems = computed(() =>
    locales.value.map((l) => ({
      label: l.name, // Changed from l.code
...
<template>
  <div class="flex w-full flex-row items-center px-3 lg:px-5 h-[72px] lg:h-20 z-40 fixed bg-white/85 dark:bg-black/85 backdrop-blur-sm dark:backdrop-blur-lg">
    <div class="flex flex-row w-full flex-nowrap items-center gap-2">
      ...
      <UDropdown :items="[languageItems]" :popper="{ placement: 'bottom-start' }" v-if="languageItems.length > 1">
        <button
            aria-label="Change language"
            class="w-12 h-12 flex items-center justify-center rounded-full cursor-pointer font-semibold text-sm text-[#5f5f5f] dark:text-[#b7b7b7] hover:bg-black/5 hover:dark:bg-white/15 max-lg:dark:bg-white/15 max-lg:bg-black/5 max-lg:hover:bg-black/10 max-lg:hover:dark:bg-white/20"
        >
          {{ currentLocale?.code }}
        </button>
      </UDropdown>
